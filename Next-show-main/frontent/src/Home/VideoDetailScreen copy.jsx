      ...
      {!isWatchingFull && (
        <>
          ...
          <video
            ...
            muted
            playsInline
            onEnded={goToNextVideo} // ⭐ Video mudinja auto-ah next pogum
          />
          ...
          {/* Text Content */}
          <div
            className={`relative z-20 flex flex-col justify-center h-screen px-6 md:px-16 max-w-2xl transition-opacity duration-700 ${
          ...
          >
            ...
            <p className="text-md mb-6">{currentVideo.description}</p>

            {currentVideo.src && (
              <button
                onClick={handleWatchNow}
                className="bg-red-600 w-[200px] py-3 rounded-lg font-bold hover:bg-red-700 transition"
              >
                Watch Now
              </button>
            )}
          ...
          {/* ⭐ Sync aagura Thumbnail Carousel */}
          <div className="absolute bottom-5 left-0 w-full px-6 md:px-16 z-30">
            <Slider ref={sliderRef} {...settings}>
              {videoPlaylist.map((item, index) => (
                <div key={index} className="px-2 outline-none">
                  <div
                    onClick={() => setCurrentIndex(index)}
                    className={`h-24 rounded-md overflow-hidden border-2 transition-all duration-300 cursor-pointer ${
                      currentIndex === index
