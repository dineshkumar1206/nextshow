...
import {
  createHomeMovieContent,
  fetchAllHomeMovies,
  updateHomeMovieContent,
} from "../../../../redux/HomeContentSlice/HomeMovieSlice";

const HomeMoviesForm = ({ isOpen, onClose, contentData, setAlert }) => {
  const dispatch = useDispatch();
  const { isLoading } = useSelector((state) => state.homeMovies);
  ...
  const handleSubmit = async (e) => {
    ...
    const action = isEdit
      ? updateHomeMovieContent({ id: contentData.id, formData: data })
      : createHomeMovieContent(data);

    dispatch(action)
      .unwrap()
      .then(() => {
        dispatch(fetchAllHomeMovies());
        onClose();
        setAlert(
          "success",
