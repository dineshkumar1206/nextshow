const HomeTrailers = require("../../models/HomePage/HomeTrailers");

// 1. அனைத்து உள்ளடக்கங்களையும் பெறுதல் (ADMIN READ)
exports.getAllHomeTrailers = async (req, res) => {
  try {
    const list = await HomeTrailers.findAll({
      order: [["order", "ASC"]],
    });
...
exports.getActiveHomeTrailers = async (req, res) => {
  try {
    const activeList = await HomeTrailers.findAll({
      where: { isActive: true },
      order: [["order", "ASC"]],
    });
...
exports.createHomeTrailers = async (req, res) => {
  try {
    ...
    const newItem = await HomeTrailers.create({
      title,
...
exports.updateHomeTrailers = async (req, res) => {
  try {
    const { id } = req.params;
    const record = await HomeTrailers.findByPk(id);
...
// 5. நீக்குதல் (DELETE)
exports.deleteHomeTrailers = async (req, res) => {
  try {
    const { id } = req.params;
    const record = await HomeTrailers.findByPk(id);
