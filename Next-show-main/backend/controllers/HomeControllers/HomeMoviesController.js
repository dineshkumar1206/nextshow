const HomeMovies = require("../../models/HomePage/HomeMovies");

// 1. அனைத்து உள்ளடக்கங்களையும் பெறுதல் (ADMIN READ)
exports.getAllHomeMovies = async (req, res) => {
  try {
    const list = await HomeMovies.findAll({
      order: [["order", "ASC"]],
    });
...
exports.getActiveHomeMovies = async (req, res) => {
  try {
    const activeList = await HomeMovies.findAll({
      where: { isActive: true },
      order: [["order", "ASC"]],
    });
...
exports.createHomeMovies = async (req, res) => {
  try {
    ...
    const newItem = await HomeMovies.create({
      title,
...
exports.updateHomeMovies = async (req, res) => {
  try {
    const { id } = req.params;
    const record = await HomeMovies.findByPk(id);
...
// 5. நீக்குதல் (DELETE)
exports.deleteHomeMovies = async (req, res) => {
  try {
    const { id } = req.params;
    const record = await HomeMovies.findByPk(id);
